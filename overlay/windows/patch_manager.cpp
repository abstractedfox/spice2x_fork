...
namespace overlay::windows {
    ...
    void PatchManager::build_content() {
        ...
        for (const auto& dll : getExtraDlls(avs::game::DLL_NAME)) {
            std::string dll_path = (MODULE_PATH / dll).string();
                    ...
                    if (patch_status == PatchStatus::Error){
                        ...
                        if (patch_status == PatchStatus::Enabled) {
                            if (patch.type == PatchType::Union) {
                                ...
                                ImGui::SetNextItemWidth(200.0f);
                                auto& numpatch = patch.patch_number;
                                // ImGuiInputTextFlags_EnterReturnsTrue is used to prevent updating
                                // too often (for both in-memory patch and the config file writing)
                                bool modified = ImGui::InputInt(
                                    "##int_input", &numpatch.value,
                                    1, 10,
    ...
    std::string get_game_identifier(const std::string& dll_path) {
        std::ifstream file(dll_path, std::ios::binary);
        if (!file) {
            log_warning("patchmanager", "Failed to open file: {}", dll_path);
            return "";
        }
        ...
        if (dos_header.e_magic != IMAGE_DOS_SIGNATURE) {
            log_warning("patchmanager", "Invalid DOS signature: {}", dll_path);
            return "";
        }
        ...
        if (nt_headers.Signature != IMAGE_NT_SIGNATURE) {
            log_warning("patchmanager", "Invalid NT signature: {}", dll_path);
            return "";
        }

        // get the TimeDateStamp and AddressOfEntryPoint from the file header
        uint32_t time_date_stamp = nt_headers.FileHeader.TimeDateStamp;
        uint32_t address_of_entry_point = nt_headers.OptionalHeader.AddressOfEntryPoint;
    ...
    bool PatchManager::import_remote_patches_for_dll(const std::string& url, const std::string& dll_name) {
        ...
        std::string identifier = get_game_identifier((MODULE_PATH / dll_name).string());
    ...
    bool PatchManager::load_from_patches_json(bool apply_patches) {
        ...
        modules.push_back(get_game_identifier((MODULE_PATH / avs::game::DLL_NAME).string()));
        for (const std::string& dll : getExtraDlls(avs::game::DLL_NAME)) {
            modules.push_back(get_game_identifier((MODULE_PATH / dll).string()));
        }
    ...
    void PatchManager::reload_local_patches(bool apply_patches) {
        ...
        std::string first_id = get_game_identifier((MODULE_PATH / firstDll).string());
        ...
        std::erase_if(extraDlls, [](const std::string& dll) {
            auto identifier = get_game_identifier((MODULE_PATH / dll).string());
        ...
        if (fileutils::file_exists(firstPath) || !extraDlls.empty()) {
            ...
            for (const std::string& dll : extraDlls) {
                auto extraId = get_game_identifier((MODULE_PATH / dll).string());
