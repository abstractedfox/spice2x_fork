...
static HWND TDJ_SUBSCREEN_WINDOW = nullptr;
                                        ...
                                        LPVOID lpParam)
{
    ...
    const std::string window_name(lpWindowName != nullptr ? lpWindowName : "");
    if (avs::game::is_model("LDJ") && GRAPHICS_WINDOWED && window_name.ends_with(" sub")) {
...
static BOOL WINAPI MoveWindow_hook(HWND hWnd, int X, int Y, int nWidth, int nHeight, BOOL bRepaint) {
    ...
    // iidx windowed TDJ mode - close the subscreen window
    // this window does not accept any input, so it's better to hide from user's
    // view to avoid any confusion. subscreen overlay should continue to work
    if (TDJ_SUBSCREEN_WINDOW && hWnd == TDJ_SUBSCREEN_WINDOW) {
        log_info(
            "graphics",
