...
std::optional<int> AUTO_INSERT_CARD_TIMEOUT;
...
bool eamuse_card_insert_consume(int active_count, int unit_id) {
    ...
    // auto insert card - turn off after timeout, if present
    if (AUTO_INSERT_CARD_TIMEOUT.has_value() && (AUTO_INSERT_CARD[0] || AUTO_INSERT_CARD[1])) {
        if (!AUTO_INSERT_CARD_FIRST_CONSUME_TIME.has_value()) {
            AUTO_INSERT_CARD_FIRST_CONSUME_TIME = get_performance_seconds();
        }
        const float seconds_since_first_read = 
            get_performance_seconds() - AUTO_INSERT_CARD_FIRST_CONSUME_TIME.value();
        if (AUTO_INSERT_CARD_TIMEOUT <= seconds_since_first_read) {
            AUTO_INSERT_CARD[0] = false;
            AUTO_INSERT_CARD[1] = false;
            CARD_INSERT[0] = false;
            CARD_INSERT[1] = false;
            log_info(
                "eamuse",
                "Disabling automatic card insert from now to prevent soft lock for this game (-autocard)");
        }
    }
    ...
    if (AUTO_INSERT_CARD[index]) {
        if (!CARD_INSERT[index]) {
            ...
            if (AUTO_INSERT_CARD_TIMEOUT.has_value()) {
                log_info(
                    "eamuse",
                    "... will be disabled after {} seconds to prevent soft lock for this game (-autocard)",
                    AUTO_INSERT_CARD_TIMEOUT.value());
            }
        }
        return true;
    }
